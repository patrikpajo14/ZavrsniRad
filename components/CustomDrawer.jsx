...
export default function CustomDrawer({isOpened, title, children, onClose}) {
    ...
    useEffect(()=>{
        isOpened ? document.body.classList.add('no-scroll') : document.body.classList.remove('no-scroll');
    }, [isOpened])

    return (
        <AnimatePresence>
            {isOpened && (
                <motion.div className="relative z-10" aria-labelledby="slide-over-title" role="dialog" aria-modal="true">
                    <motion.div
                            ...
                            <div className="flex h-full flex-col overflow-y-scroll bg-white py-6 shadow-xl max-h-[calc(100vh - 80px)] overflow-y-auto">
                                <div className="px-4 sm:px-6 flex justify-between items-center">
                                    ...
                                    <IconButton onClick={onClose}>
                                        <Image src={"/assets/icons/ico_close.svg"} alt={"close"} width={20} height={20} />
                                    </IconButton>
                                </div>
                                <motion.div
                                    variants={itemVariants}
