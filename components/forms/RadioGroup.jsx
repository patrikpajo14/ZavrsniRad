...
export default function RadioGroup({title, inputs, defaultValue, handleOnChange}) {
            ...
            <div className="flex items-center justify-between gap-2">
                {inputs.map((input, index) =>(
                    <div key={index} className="flex items-center gap-x-2">
                        ...
                        <label htmlFor={input.id} className="block text-sm font-medium leading-6 text-gray-900 capitalize cursor-pointer">
                            {input.value}
                        </label>
                    </div>
                ))}
